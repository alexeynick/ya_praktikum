...
Модуль 4:
- [Спринт 13. Извлечение данных](#спринт-13)
- [Спринт 14. Компьютерное зрение](#спринт-14)
...
- Цели и задачи: Разработка модели для прогнозирования количества заказов такси на следующий час на основе исторических данных о заказах такси в аэропортах. Целевая метрика -  значением метрики RMSE на тестовой выборке не больше 48.   
- Описание: Задача регрессии. Временные ряды. Семплирование данных. Обучение моделей с подбором гиперпараметров, проверка на адекватность. Оценка значимости обучающих признаков. Анализ временных характеристик моделей. Анализ остатков и их распределений.
- Tags: pandas, plotly, LinearRegression, RandomForestRegressor, CatBoostRegressor, LGBMRegressor, gridsearch, cross-validation, dummy, rmse


### Спринт 12
**Обучение для текстов. Проект для модерации комментариев в «Викишоп».**
- Цели и задачи: Цель проекта - обучить модель классифицировать комментарии на позитивные и негативные. В нашем распоряжении набор данных с разметкой о токсичности правок. Целевая метрика - F1-меры не менее 0.75.
- Описание: Задача классификации. Анализ текстов, токенизация, лемматизация. Матрица TF-IDF. N-граммы. Обучение моделей с подбором гиперпараметров. Балансировка целевых классов. График ROC-AUC, настройка порогов отнесения к целевым классам. Оценка значимости обучающих признаков. Анализ остатков.
- Tags: pandas, plotly, LogisticRegression, LinearSVC, CatBoostClassifier, pipeline, gridsearch, cross-validation, roc-auc, f1, precision, recall

---------------------------------------------------------------------------------------------------

### Спринт 13
**Извлечение данных. Анализ спроса пассажиров на авиаперевозки.**
- Цели и задачи: Визуализировать данные по авиарейсам за определенный период.
- Описание: Выявление ТОП-10 городов по посещаемости среди данных по авиаперелетам.
- Tags: pandas, plotly, предобработка данных

### Спринт 14
**Компьютерное зрение. Определение возраста по фотографии.**
- Цели и задачи: Построить модель, которая по фотографии определит приблизительный возраст человека.
- Описание: Настройка модели ResNet-50, аугментации, полносвязные слои.
- Tags: pandas, plotly, tensorflow, keras, mae
